...
exports.getRealtimelog = async(req, res, next) => {
    ...
    try{
        ...
        res.render("log/realtimelog", {locList: row2, liRow: paginate.pages[paginate.page - 1], pages: paginate.threePages});
    }catch(err) {
        console.log(err);
...
*/
        ...
        res.render("log/realtimedatalog",{btry_seq:info.btry_seq});
    }catch(err) {
        console.log(err);
        res.send({locList : null, liRow : "1", pages : "1"});
    }
