...
const viewName = 'production/report/';
require("date-utils");
...
exports.postOperCmpyList = async(req, res, next) => {
    //장소별 bms list
    console.log("start postOperCmpyList");
    try {

        var cmpy_seq = req.session.userInfo.cmpy_seq;

        let cmpy_list = await common.getBtryGroupByCmpy(cmpy_seq);

        for(var i in cmpy_list){
            if(cmpy_list[i].cmpy_nm == "전체"){
                cmpy_list.splice(i,1);
                break;
            }
        }

        res.send({cmpy_list : cmpy_list});
...
exports.postOperCmpyListWithAll = async(req, res, next) => {
    //장소별 bms list
    console.log("start postOperCmpyListWithAll");
    try {
...
exports.postLocListByCmpyWithAll = async(req, res, next) => {
    //장소별 bms list
    console.log("start postLocListByCmpyWithAll");
    try {
...
exports.postBtryTypeList = async(req, res, next) => {
    //장소별 bms list
    console.log("start postBtryListByLoc");
    try {
