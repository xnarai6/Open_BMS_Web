...
const topcart = require(path.join(global.appRoot, 'routes/golf/common/topcart.js'));

router.get('/', (req, res) => { res.redirect('/golf/dashboard'); });
...
router.use(async (req, res, next) => {
    ...
    // role 체크
    let result = await role.checkRole(req, res, '/golf', 'back');
    if (!result) return ;
