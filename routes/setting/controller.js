...
exports.getAcnt = async(req, res, next) => {
    ...
    try {
        ...
        if(row2.length <= 0) {
            ...
            res.render("setting/acnt",{ACNT_LIST : row2, liRow : paginate.pages[paginate.page - 1], pages : paginate.threePages});
        }
    } catch (error) {
...
exports.getAcntReg = async(req, res, next) => {
    ...
    try{
        ...
        res.render("setting/acntReg",{cmpyTyList : row1, roleList : row2, cmpyList: row3});
    }catch (error) {
        console.log(error);
        res.render("setting/acntReg",{cmpyTyList : null, roleList : null, cmpyList: null});
    }
...
exports.getAcntMod = async(req, res, next) => {
    ...
    try{
        ...
        res.render("setting/acntMod",{acntInfo : row, cmpyTyList : row1, roleList : row2, cmpyList: row3});
    }catch (error) {
        console.log(error);
        res.render("setting/acntMod",{acntInfo : null, cmpyTyList : null, roleList : null, cmpyList: null});
    }
...
exports.getCmpy = async(req, res, next) => {
    ...
    try {
        ...
        if(row2.length <= 0) {
            ...
            res.render("setting/cmpy",{CMPY_LIST : row2, liRow : paginate.pages[paginate.page - 1], pages : paginate.threePages});
        }
    } catch (error) {
...
exports.getCmpyReg = async(req, res, next) => {
    ...
    try {
        ...
        res.render("setting/cmpyReg", {cmpy_ty_list : row1, cmpy_div_list : row2, cmpy_biz_type_list : row3, acnt_list : row4, btry_list : row5, loc_list : row6});
...
exports.getCmpyMod = async(req, res, next) => {
    ...
    try {
        ...
        res.render("setting/cmpyMod", {cmpyInfo : row, cmpy_ty_list : row1, cmpy_div_list : row2, cmpy_biz_type_list : row3, acnt_list : row4, cmpy_acnt_list : row4_selected, btry_list : row5, cmpy_btry_list : row5_selected, loc_list : row6, cmpy_loc_list :row6_selected});
...
exports.getMgtlocation = async(req, res, next) => {
    try {
        ...
        if(row2.length <= 0) {
            ...
            res.render("setting/mgtlocation",{LOC_LIST : row2, liRow : paginate.pages[paginate.page - 1], pages : paginate.threePages});
        }
    } catch (error) {
        console.log(error);
    }
...
exports.getMgtlocationReg = async(req, res, next) => {
    ...
    try {
        ...
        res.render("setting/mgtlocationReg", {cmpy_list : row1, btry_list : row2});
...
exports.getMgtlocationMod = async(req, res, next) => {
    ...
    try {
        ...
        res.render("setting/mgtlocationMod",{locInfo : row, cmpy_list : row1, btry_list : row2, loc_btry_list : row3});
...
exports.getMgtmodule = async(req, res, next) => {
    try {
        ...
        if(row2.length <= 0) {
            ...
            res.render("setting/mgtmodule",{MDL_LIST : row2, liRow : paginate.pages[paginate.page - 1], pages : paginate.threePages});
        }
    } catch (error) {
        console.log(error);
    }
...
exports.getMgtmoduleReg = async(req, res, next) => {
    try {
        ...
        res.render("setting/mgtmoduleReg",{mdl_ty_list : row1, btry_mdl_ty_list : row2, btry_ty_list : row3});
...
exports.getMgtmoduleMod = async(req, res, next) => {
    try {
        ...
        res.render("setting/mgtmoduleMod",{mdl_ty_list : row1, btry_mdl_ty_list : row2, btry_ty_list : row3, mdlInfo : row});
...
exports.getBatteryList = async(req, res, next) => {
    let session = req.session;

    // 1. cd 정보 가져오기
    let selectQuery1 = 'SELECT tc.gp_cd, tc.cd, tc.cd_desc';
    selectQuery1 += ' FROM TBL_CD tc';
    selectQuery1 += ' WHERE tc.gp_cd IN ("btry_ty", "btry_mdl_ty")';

    let [rows1] = await global.mysqlPool.query(selectQuery1);

    // 2. 배터리 리스트 가져오기
    let selectQuery2 = 'SELECT tb.btry_seq, tb.btry_nm, "LI" AS btry_ty_cd,';
    selectQuery2 += ' tb.btry_mfctor_nm, btry_max_pwr,';
    selectQuery2 += ' DATE_FORMAT(tb.ins_dttm, "%Y-%m-%d %H:%i:%S") AS ins_dttm, tb.ins_nm';
    selectQuery2 += ' FROM OPENBMS.TBL_BTRY tb'; 
    // selectQuery2에 자기 배터리만 가져오도록 where 추가 필요함

    let [rows2] = await global.mysqlPool.query(selectQuery2);
    
    res.render('setting/batteryList', {
        codeList: rows1,
        batteyList: rows2
    });
}
...
exports.getBatteryDetail = async(req, res, next) => {
    ...
    // 1. param 가져오기
    let batterySeq = req.params.batterySeq;
    ...
    selectQuery2 += ' DATE_FORMAT(tb.ins_dttm, "%Y-%m-%d %H:%i:%S") AS ins_dttm, tb.ins_nm';
    selectQuery2 += ' FROM OPENBMS.TBL_BTRY tb'; 
    selectQuery2 += ' WHERE tb.btry_seq = "' + batterySeq + '"'; 

    let [rows2] = await global.mysqlPool.query(selectQuery2);
    
    res.render('setting/batteryDetail', {
        batterySeq: batterySeq,
        codeList: rows1,
        batteyInfo: rows2
    });
}

exports.getBatteryFormat = async (req, res, next) => {
    console.log("TTT1");
    let session = req.session;

    // 1. param 가져오기
    const batterySeq = req.params.batterySeq;
    
    res.render('setting/batteryFormat', {
        batterySeq: batterySeq
    });
}

exports.postFormatData = async (req, res, next) => {
    console.log("TTT2");
    let session = req.session;
    ...
    // 2. 배터리 프로토콜 포맷 정보 가져오기
    let selectQuery1 = 'SELECT tpf.ptc_fmt_seq, tpf.cmpy_seq, tpf.btry_seq, tpf.brd_num,';
        selectQuery1 += ' tpf.ptc_fmt_json, tpf.db_fmt_json,';
        selectQuery1 += ' DATE_FORMAT(tpf.ins_dttm, "%Y-%m-%d %H:%i:%S") AS ins_dttm, tpf.ins_nm,';
        selectQuery1 += ' DATE_FORMAT(tpf.upd_dttm, "%Y-%m-%d %H:%i:%S") AS upd_dttm, tpf.upd_nm';
        selectQuery1 += ' FROM OPENBMS.TBL_PTC_FMT tpf'; 
        selectQuery1 += ' WHERE tpf.btry_seq = "' + batterySeq + '"';

    console.log(selectQuery1);

    const [rows1] = await global.mysqlPool.query(selectQuery1);

    let ptcFormat = JSON.parse(rows1[0].ptc_fmt_json);

    res.json({ data: Object.entries(ptcFormat) });
}
exports.getMgtbattery = async(req, res, next) => {
    ...
    try {
        ...
        // 로그인 따른 BMS 리스트
        let sql2 = " select tb.*, (SELECT tm.mdl_no FROM OPENBMS.TBL_MDL tm WHERE tm.mdl_delete_yn = 'N' AND tm.mdl_seq = tb.mdl_seq) as mdl_no, ";
            sql2 +=" (SELECT tc.cd_desc FROM OPENBMS.TBL_CD tc WHERE tc.cd = (SELECT tm.btry_ty FROM OPENBMS.TBL_MDL tm WHERE tb.mdl_seq = tm.mdl_seq)) as btry_ty, ";
            sql2 +=" (SELECT tc.cmpy_nm FROM OPENBMS.TBL_CMPY tc WHERE tc.cmpy_seq = (SELECT mcl.cmpy_seq FROM OPENBMS.MPP_CMPY_LOC mcl WHERE mcl.loc_seq = (SELECT mlb.loc_seq FROM OPENBMS.MPP_LOC_BTRY mlb WHERE mlb.btry_seq = tb.btry_seq))) as cmpy_nm"; 
        ...
        if(row2.length <= 0) {
            ...
            res.render("setting/mgtbattery",{BMS_LIST : row2, liRow : paginate.pages[paginate.page - 1], pages : paginate.threePages});
        }
    } catch (error) {
...
exports.getMgtbatteryReg = async(req, res, next) => {
    ...
    try {
        ...
        res.render("setting/mgtbatteryReg",{btry_mdl_ty_list : row2, btry_stat_list : row3_1, btry_mng_stat_list : row3_2, cmpy_list : row4, mdl_list : row5, loc_list : row6});
...
exports.getMgtbatteryMod = async(req, res, next) => {
    ...
    try {
        ...
        res.render("setting/mgtbatteryMod",{btryInfo : row, btry_mdl_ty_list : row2, btry_stat_list : row3_1, btry_mng_stat_list: row3_2, cmpy_list : row4, mdl_list : row5, loc_list : row6});
...
exports.getMgtalarm = async(req, res, next) => {
    ...
    try {
        ...
        //페이지네이션을 위한 전체 카운트
        let sql1 = "SELECT COUNT(*) AS total_count FROM( ";
            sql1 += " select ta.alm_seq, ta.btry_seq, ta.alm_delete_yn,";
            ...
            sql1 +=" ta.ins_nm, ta.ins_dttm, ta.upd_nm, ta.upd_dttm, ta.alm_sms, ta.stop_pwr ";
            sql1 +=" FROM OPENBMS.TBL_ALM ta  ";
            sql1 +=" LEFT OUTER JOIN OPENBMS.TBL_BTRY tb2 ON tb2.btry_seq = ta.btry_seq ";
            sql1 +=" WHERE ta.alm_delete_yn = 'N' ";
            sql1 +=" and ta.btry_seq in (select btry_seq from OPENBMS.MPP_CMPY_BTRY  mcb ";
            sql1 +=" where mcb.cmpy_seq = (select cmpy_seq from OPENBMS.TBL_ACNT where acnt_seq = "+ sess.acnt_seq +"))";
        ...
        // 로그인 따른 BMS 리스트
        let sql2 = " select ta.alm_seq, ta.btry_seq, ta.alm_delete_yn,";
        ...
        sql2 +=" ta.ins_nm, ta.ins_dttm, ta.upd_nm, ta.upd_dttm, ta.alm_sms, ta.stop_pwr ";
        sql2 +=" FROM OPENBMS.TBL_ALM ta  ";
        sql2 +=" LEFT OUTER JOIN OPENBMS.TBL_BTRY tb2 ON tb2.btry_seq = ta.btry_seq ";
        sql2 +=" WHERE ta.alm_delete_yn = 'N' ";
        sql2 +=" and ta.btry_seq in (select btry_seq from OPENBMS.MPP_CMPY_BTRY  mcb ";
        sql2 +=" where mcb.cmpy_seq = (select cmpy_seq from OPENBMS.TBL_ACNT where acnt_seq = "+ sess.acnt_seq +"))";
        ...
        res.render("setting/mgtalarm",{ALM_LIST : row2, liRow : paginate.pages[paginate.page - 1], pages : paginate.threePages});
...
exports.getMgtalarmReg = async(req, res, next) => {
    try {
        ...
        res.render("setting/mgtalarmReg",{btry_list : row1});
...
exports.getMgtalarmMod = async(req, res, next) => {
    ...
    try {
        ...
        let sql = "SELECT ta.*,(SELECT tb.btry_nm FROM TBL_BTRY tb WHERE ta.btry_seq = tb.btry_seq) as btry_nm FROM OPENBMS.TBL_ALM ta WHERE ta.alm_delete_yn = 'N' AND ta.alm_seq = " + info.alm_seq;
        let [row] = await global.mysqlPool.query(sql);
        ...
        res.render("setting/mgtalarmMod",{ALM_INFO : row, btry_list : row1});
...
exports.getAlarmReg = async(req, res, next) => {
    ...
    res.render("setting/alarmReg", {btry_seq:info.btry_seq});
}
