...
const viewName = 'smartpole/manage/';
...
exports.postUserList = async (req, res, next) => {
    ...
    if (!Number.isInteger(page)) return form.result(res, req.method, 'FAIL', null, '잘못된 파라미터입니다', null, '/smartpole/dashboard');
    if (page <= 0) page = 1;

    // 3. 회사번호에 해당되는 사용자 리스트 가져오기
    let userData = await commonManage.getUserList(cmpySeq, page, 10);
    if (!userData) return form.result(res, req.method, 'FAIL', null, '해당되는 데이터가 없습니다', null, '/smartpole/dashboard');
...
exports.getUserDetail = async (req, res, next) => {
    ...
    if (!Number.isInteger(seq)) return form.result(res, req.method, 'FAIL', null, '잘못된 파라미터입니다', null, '/smartpole/manage/user');
...
exports.postUserDetailInfo = async (req, res, next) => {
    ...
    if (!Number.isInteger(targetSeq)) return form.result(res, req.method, 'FAIL', null, '잘못된 파라미터입니다', null, '/smartpole/manage/user');

    // 4. 자기 회사에 속한 직원인지 확인
    let userDetailInfo = await commonManage.getUserDetail(cmpySeq, targetSeq);
    if (!userDetailInfo) return form.result(res, req.method, 'FAIL', null, '해당되는 데이터가 없습니다', null, '/smartpole/manage/user');
...
exports.postUserDetailChg = async (req, res, next) => {
    ...
    // 10. 완료
    return form.result(res, req.method, 'SUCCESS', null, '사용자 수정이 완료되었습니다', null, '/smartpole/manage/user');
}
...
exports.postUserRegChk = async (req, res, next) => {
    ...
    // 10. 사용자 생성
    let result = await commonManage.insUser(acntId, cmpySeq, body, 'smartpole');
    if (!result) form.result(res, req.method, 'FAIL', null, '사용자 생성 실패', null, null);

    // 11. 완료
    return form.result(res, req.method, 'SUCCESS', null, '사용자 등록이 완료되었습니다', null, '/smartpole/manage/user');
}
...
exports.postUserProfileInfo = async (req, res, next) => {
    ...
    if (!userProfileInfo) return form.result(res, req.method, 'FAIL', null, '해당되는 데이터가 없습니다', null, '/smartpole/manage/user');
...
exports.postUserProfileChg = async (req, res, next) => {
    ...
    // 9. 완료
    return form.result(res, req.method, 'SUCCESS', null, '사용자 수정이 완료되었습니다', null, '/smartpole/dashboard');
}
...
exports.postUserProfileChgPw = async (req, res, next) => {
    ...
    // 9. 완료
    return form.result(res, req.method, 'SUCCESS', null, '비밀번호 수정이 완료되었습니다', null, '/smartpole/dashboard');
}